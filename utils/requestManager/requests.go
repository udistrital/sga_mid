...
// TODO: de aquí en adelante, en proceso de desarrollo, aún no probado, No borrar
/*
var inspectRequest bool

// Indica si desea ver el proceso de la petición, url y respuesta
//   - inspect: true para si
func InspectRequest(inspect bool) {
	inspectRequest = inspect
}

var bearerToken string

// Si petición requiere authorization,aquí pasa bearer token proveniente de cliente
//   - h: header del controlador mid que recibe
func PassBearer(h http.Header) {
	bearerToken = h.Get("Authorization")
}

// Pasar el bearer token a la petición, usado desde _doReq
//   - req: request actual
func _setBeaer(req *http.Request) {
	if bearerToken != "" {
...
// ? Para hace peticiones de manera asíncrona mediante go routines

func AsyncPost(url string, body interface{}, parser func(interface{}) (interface{}, error), ch chan<- Result) {
	resp, err := Post(url, body, parser)
	ch <- Result{Response: resp, Error: err}
}

func AsyncGet(url string, parser func(interface{}) (interface{}, error), ch chan<- Result) {
	resp, err := Get(url, parser)
	ch <- Result{Response: resp, Error: err}
}

func AsyncPut(url string, body interface{}, parser func(interface{}) (interface{}, error), ch chan<- Result) {
	resp, err := Put(url, body, parser)
	ch <- Result{Response: resp, Error: err}
}

func AsyncDelete(url string, parser func(interface{}) (interface{}, error), ch chan<- Result) {
	resp, err := Delete(url, parser)
	ch <- Result{Response: resp, Error: err}
}
*/
