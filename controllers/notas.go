...
// @Success 200 {}
// @Failure 403 :id_docente is empty
// @router /listaEspaciosAcademicos/:id_docente [get]
func (c *NotasController) GetEspaciosAcademicosDocente() {
	...
	if fmt.Sprintf("%v", id_docente) != "" {
		...
		if errEspaciosAcademicosRegistros == nil && fmt.Sprintf("%v", EspaciosAcademicosRegistros["Data"]) != "[]" {
			...
			if errNiveles == nil && fmt.Sprintf("%v", niveles[0]) != "map[]" {
				...
				if errCalendario == nil && fmt.Sprintf("%v", calendarios[0]) != "map[]" {
					...
					if errPeriodos == nil && fmt.Sprintf("%v", periodos["Status"]) == "200" {
...
// @Success 200 {}
// @Failure 403 :id_asignatura is empty
...
func (c *NotasController) GetDatosDocenteAsignatura() {
	...
	if fmt.Sprintf("%v", id_asignatura) != "" {
...
func (c *NotasController) GetPorcentajesAsignatura() {
	...
	if fmt.Sprintf("%v", id_asignatura) != "" {
		...
		if errRegistroAsignatura == nil && fmt.Sprintf("%v", RegistroAsignatura["Status"]) == "200" && fmt.Sprintf("%v", RegistroAsignatura["Data"]) != "[]" {
